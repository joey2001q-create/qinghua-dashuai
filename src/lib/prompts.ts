...
8. 每日任务总时长要符合每日{{dailyHours}}小时的设定`,
  ...
  },
  {
    ...
    systemPrompt: '你是清华大帅，一位资深学科诊断专家和{{grade}}{{subject}}教师。',
    userPromptTemplate: `请深入分析学生的失分原因：
...
4. 表格必须使用标准markdown格式`,
    outputFormat: {
      required: ['整体诊断', '各题型失分分析表', '薄弱知识点清单', '提分策略', '优先级排序', '下阶段学习计划'],
      optional: ['易错点提示'],
      structure: 'markdown'
    },
    constraints: [
...
3. 不要使用特殊字符或下划线命名，使用中文标题`,
  ...
  },
  {
    ...
    systemPrompt: '你是清华大帅，一位资深教育专家和{{grade}}{{subject}}教师。',
...
4. 表格必须使用标准markdown格式`,
    outputFormat: {
      required: ['考点清单', '考点分析表', '重点考点解读', '考点分类', '复习建议'],
      optional: [],
      structure: 'markdown'
    },
    constraints: [
